...
const UnrestPage: React.FC = () => {
  ...
  const {
    ...
    handleSubmit,
    isSubmitting,
  } = useSelfTests(2); // 2는 불안 테스트의 ID라고 가정합니다.
  ...
  if (!selfTestList) return <div>No test information available</div>;

  const unrestInfo = selfTestList.find(info => info.selftestName === 'unrest');

  const onSubmit = () => {
    const result = handleSubmit(isPublic ? 1 : 0);
    if (result && !result.error) {
      navigate('/selftest/unrest/result', {
        state: {
